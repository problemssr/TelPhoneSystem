...
cur = conn.cursor()
phone = load_workbook('media/20220822手机资料.xlsx')
sheet = phone['上架手机118部']
...
for row in sheet.iter_rows(min_row=2, min_col=1, max_col=108):
    cur.execute(
        ...
        "detail_remark,detail_power_specification,detail_power_manufacturer,"
        "detail_Model,detail_alias,detail_IMEI,detail_charging,detail_wireless_charging,detail_purchase_date) "
        "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
